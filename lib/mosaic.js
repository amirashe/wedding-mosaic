...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  // 7. Ghost overlay - blend target image at low opacity for extra clarity
  ...
  const mosaicRaw = await sharp(mosaic)
    .raw()
    .toBuffer()

  // Blend manually - fast and reliable
  const blended = Buffer.from(mosaicRaw)
  const iG = 1 - GHOST
  for (let i = 0; i < blended.length; i++) {
    blended[i] = Math.round(blended[i] * iG + ghostBuf[i] * GHOST)
  }
