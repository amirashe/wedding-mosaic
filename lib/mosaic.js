...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  const lastPosition = new Map()   // track where each photo was last placed
  ...
  for (const tile of tiles) {
    ...
    if (bestPos >= 0) {
      ...
      usage.set(tile.id, 1)
      lastPosition.set(tile.id, bestPos)
    }
  }
  ...
  for (let pos = 0; pos < total; pos++) {
    ...
    for (const pass of [0, 1, 2]) {
      for (const t of tiles) {
        ...
        if (pass < 1) {
          const lastPos = lastPosition.get(t.id)
          if (lastPos !== undefined) {
            const lx = lastPos % GRID_SIZE
            const ly = Math.floor(lastPos / GRID_SIZE)
            const dist = Math.max(Math.abs(x - lx), Math.abs(y - ly))
            if (dist < MIN_REUSE_DIST) continue
          }
        }
