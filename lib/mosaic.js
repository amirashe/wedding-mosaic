...
async function getAverageColor(buffer) {
  const jpeg = await sharp(buffer).resize(1, 1).jpeg().toBuffer()
...
async function makeVariation(buffer, index) {
  ...
  // Convert to JPEG first → guarantees exactly 3 channels (RGB)
  ...
  // Then extract raw RGB bytes
  return sharp(jpeg).raw().toBuffer()
}
...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  for (let i = 0; i < tiles.length; i++) {
    ...
    if (vars.length === 0) {
      try {
        const fb = await sharp(tiles[i].buffer).resize(TILE_SIZE, TILE_SIZE, { fit: 'cover' }).jpeg({ quality: 85 }).toBuffer()
        vars.push(await sharp(fb).raw().toBuffer())
      } catch {}
    }
  ...
  for (let strip = 0; strip < NUM_STRIPS; strip++) {
    ...
    }).composite(stripOverlays).png({ compressionLevel: 0 }).toBuffer()

    stripBuffers.push({ buf, top: startRow * TILE_SIZE })
  }

  // 8. Join strips into final image (encode as JPEG explicitly)
