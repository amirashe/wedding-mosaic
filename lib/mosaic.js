...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  // 7. Composite + ghost overlay
  const mosaic = await sharp({
    create: { width: OUTPUT_SIZE, height: OUTPUT_SIZE, channels: 3, background: { r: 20, g: 20, b: 20 } }
  }).composite(overlays).jpeg({ quality: 92 }).toBuffer()

  const ghostBuf  = await sharp(targetBuf).resize(OUTPUT_SIZE, OUTPUT_SIZE).removeAlpha().raw().toBuffer()
  ...
  return sharp(blended, { raw: { width: OUTPUT_SIZE, height: OUTPUT_SIZE, channels: 3 } })
