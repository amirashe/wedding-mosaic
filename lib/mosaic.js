...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  // Convert to B&W + sharpen + contrast → better matching for similar-toned photos
  const targetProcessed = await sharp(targetBuf)
    .greyscale()
    .toColorspace('srgb')          // keep 3 channels for LAB conversion
  ...
  for (let pos = 0; pos < total; pos++) {
    ...
    // Center tiles demand strict matching; edge tiles are more lenient
    const weight    = centerWeight(pos, GRID_SIZE)
    const tolerance = (1 - weight) * 8000   // center=0, edges=4800 extra tolerance
    ...
    for (const pass of [true, false]) {
      for (const t of tiles) {
        ...
        // At edges, allow picking from wider candidate pool via tolerance
  ...
  onProgress?.('composite', NUM_STRIPS, NUM_STRIPS)
  ...
  // 7. Adaptive ghost overlay - higher opacity at center (face), lower at edges
  ...
  // Precompute opacity per row (cache dy² to avoid redundant computation)
  ...
  for (let py = 0; py < OUTPUT_SIZE; py++) {
    ...
    for (let px = 0; px < OUTPUT_SIZE; px++) {
      const t       = Math.sqrt((px - cx) ** 2 + dy2) / maxDist
      const opacity = 0.30 - t * 0.22        // 0.30 center → 0.08 edge
      const iO      = 1 - opacity
