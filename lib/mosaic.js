...
function applyTint(rawBuf, tc, blend) {
...
export async function generateMosaic(targetUrl, tileEntries, onProgress, userParams = {}) {
  const { blend, varsPerImg, minReuseDist, ghostOpacity, jpegQuality, gridSize } = { ...DEFAULTS, ...userParams }
  const outputSize = gridSize * GRID_STEP
    ...
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        ...
        if (nx >= 0 && nx < gridSize && ny >= 0 && ny < gridSize) {
          neighbours.add(assignments[ny * gridSize + nx]?.id)
        }
      }
    }

    const weight    = centerWeight(pos, gridSize)
        ...
        if (pass < 1) {
          ...
          if (prevPositions) {
            const tooClose = prevPositions.some(prevPos => {
              const lx = prevPos % gridSize
              const ly = Math.floor(prevPos / gridSize)
              return Math.max(Math.abs(x - lx), Math.abs(y - ly)) < minReuseDist
            })
  ...
  for (let pos = 0; pos < total; pos++) {
    ...
    const tinted = applyTint(raw, targetColors[pos], blend)

    overlays.push({
      input: tinted,
      raw:   { width: TILE_SIZE, height: TILE_SIZE, channels: 3 },
      left:  (pos % gridSize) * GRID_STEP,
      top:   Math.floor(pos / gridSize) * GRID_STEP,
    })
  ...
  const NUM_STRIPS     = Math.ceil(gridSize / ROWS_PER_STRIP)
  const strips         = []
  ...
  for (let s = 0; s < NUM_STRIPS; s++) {
    ...
    const endRow   = Math.min(startRow + ROWS_PER_STRIP, gridSize)
    ...
    const stripBuf = await sharp({
      create: { width: outputSize, height: stripH, channels: 3, background: { r: 30, g: 30, b: 30 } }
    })
  ...
  const mosaic = await sharp({
    create: { width: outputSize, height: outputSize, channels: 3, background: { r: 30, g: 30, b: 30 } }
  })
    .composite(strips)
    .jpeg({ quality: 92 })
    .toBuffer()
  ...
  const tileOpacity = new Float32Array(gridSize * gridSize)
  for (let pos = 0; pos < gridSize * gridSize; pos++) {
    const w = centerWeight(pos, gridSize)
    tileOpacity[pos] = ghostOpacity - (1 - w) * (ghostOpacity - 0.08)   // ghostOpacity center → 0.08 edge
  }

  // Apply blending using tile-level opacity lookup
  for (let py = 0; py < outputSize; py++) {
    const ty      = Math.min(Math.floor(py / GRID_STEP), gridSize - 1)
    const rowBase = py * outputSize * 3
    for (let px = 0; px < outputSize; px++) {
      const tx      = Math.min(Math.floor(px / GRID_STEP), gridSize - 1)
      const opacity = tileOpacity[ty * gridSize + tx]
      const iO      = 1 - opacity
