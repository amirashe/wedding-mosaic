...
const BLEND        = 0.35    // stronger tint → color zones read better from distance
const GHOST        = 0.20
const VARS_PER_IMG    = 5    // more crop diversity per photo
const MIN_REUSE_DIST  = 10   // wider spread → less face clustering
...
export async function generateMosaic(targetUrl, tileEntries, onProgress) {
  ...
  for (let pos = 0; pos < GRID_SIZE * GRID_SIZE; pos++) {
    ...
    tileOpacity[pos] = 0.25 - (1 - w) * 0.17   // 0.25 center → 0.08 edge
  }
