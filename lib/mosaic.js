...
export async function generateMosaic(targetUrl, tileEntries) {
  ...
  // ── Build composite ──────────────────────────────────────────────────────
  const BLEND_OPACITY = 0.38   // 0 = no tint, 1 = full color
  ...
  for (let pos = 0; pos < GRID_SIZE * GRID_SIZE; pos++) {
    ...
    // Color tint overlay to match target
    const tintPng = await sharp({
      create: {
        width:    TILE_SIZE,
        height:   TILE_SIZE,
        channels: 4,
        background: { r: tc.r, g: tc.g, b: tc.b, alpha: BLEND_OPACITY }
      }
    }).png().toBuffer()

    const blended = await sharp(resized)
      .composite([{ input: tintPng, blend: 'over' }])
      .jpeg({ quality: 85 })
      .toBuffer()

    overlays.push({ input: blended, left: x, top: y })
  }
